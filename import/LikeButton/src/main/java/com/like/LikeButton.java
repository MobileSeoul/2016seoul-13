...
public class LikeButton extends FrameLayout implements View.OnClickListener {
    ...
    private Drawable getDrawableFromResource(TypedArray array, int styleableIndexId)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return (-1 != id) ? getContext().getDrawable(id) : null;
        }else{
            return (-1 != id) ? getContext().getResources().getDrawable(id) : null;
        }
    }
    ...
    public void setLikeDrawableRes(int resId) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            likeDrawable = getContext().getDrawable(resId);
        }else{
    ...
    public void setUnlikeDrawableRes(int resId) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            unLikeDrawable = getContext().getDrawable(resId);
        }else{
            unLikeDrawable = getContext().getResources().getDrawable(resId);
        }
